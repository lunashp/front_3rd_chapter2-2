...
describe("advanced > ", () => {
  ...
  describe("productUtil 테스트", () => {
    test("updateProductField는 제품 필드를 업데이트해야 한다", () => {
      ...
      expect(updatedProduct).not.toBe(mockProduct); // 얕은 복사 확인
    });

    test("findProductById는 주어진 ID에 해당하는 제품을 찾아야 한다", () => {
      const foundProduct = findProductById(mockProducts, "p1");
      expect(foundProduct).toEqual(mockProduct);
    });

    test("findProductById는 존재하지 않는 ID일 경우 undefined를 반환해야 한다", () => {
      const products = [mockProduct];
      const foundProduct = findProductById(products, "p2");
      expect(foundProduct).toBeUndefined();
    });

    test("addDiscountToProduct는 제품에 새로운 할인을 추가해야 한다", () => {
      const newMockProduct = { ...mockProduct, discounts: [] }; // 할인 배열을 빈 배열로 초기화
      const discount = { quantity: 10, rate: 0.1 };
      const updatedProduct = addDiscountToProduct(newMockProduct, discount);
      expect(updatedProduct.discounts.length).toBe(1);
      expect(updatedProduct.discounts[0]).toEqual(discount);
    });

    test("removeDiscountFromProduct는 인덱스에 해당하는 할인을 제거해야 한다", () => {
      const productWithDiscounts = {
        ...mockProduct,
        discounts: [{ quantity: 10, rate: 0.1 }],
      };
      const updatedProduct = removeDiscountFromProduct(productWithDiscounts, 0);
      expect(updatedProduct.discounts.length).toBe(0);
    });

    test("createProductWithId는 새로운 ID를 가진 제품을 생성해야 한다", () => {
      ...
      const newProduct = createProductWithId(productWithoutId);
